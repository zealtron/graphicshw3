...
{
    REAL	scale,	/* a scale factor for X1 */
    ...
    for (scale=0, k=i; k<m; k++) scale += ((REAL) fabs(REF2D(a,m,n,k,j)));
    ...
    ** is to reduce roundoff error.  */
    f = hv[i];
    snorm = ((REAL) ((f > 0) ? -sqrt(sigma) : sqrt(sigma)));
...
{
    REAL scale, sigma,snorm, f, h, s, dot;
    int k,l;

    for (scale=0, k=j; k<n; k++) scale += ((REAL) fabs(REF2D(a,m,n,i,k)));
    if (!scale) return;
...
{
    int i;
    REAL r, x, hypot, cos, sin;

    for (i=p+1; i<=z; i++) {
	r = REF2D(b,m,n,p,i);
	x = REF2D(b,m,n,i,i);
	if (r==0) break;
	hypot = ((REAL) sqrt (r*r + x*x)); //hypot = pythag(r,x);
	cos = x/hypot;
	sin = r/hypot;
...
{
    int i,j, min=MIN(m,n);
    REAL *usave = u, *vsave = v, *h = ALLOC1D(MAX(m,n));
...
void bidiagonal_svd(REAL *b,int m,int n,REAL *u,REAL *v)
{
    ...
    */
    ...
    {
	    t = ((REAL) (fabs(REF2D(b,m,n,i,i)) + fabs(REF2D(b,m,n,i-1,i))));
...
///	if (iter>=30)
//	    (void) fprintf(stderr,"svd: didn't converge after 30 iterations\n");
    }
}
		               ...
		               const REAL b[], REAL x[], REAL eps)
{
    ...
    const REAL thresh = eps * w[0];	/* singular vals are sorted, w[0] is max */
    REAL *tmp = ALLOC1D(min);
    int i,j,k;
